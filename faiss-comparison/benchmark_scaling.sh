...
PYEOF
    ...
    if command -v python3 &> /dev/null; then
        output=$(./venv/bin/python benchmark_faiss_temp.py 2>&1)
        ...
        echo -e "${RED}Python3 not found, skipping FAISS benchmark${NC}"
    fi
...
echo "Nodes     | Hippocampus | FAISS      | Winner"
echo "----------|-------------|------------|------------------"
for SCALE in "${SCALES[@]}"; do
    hippo=${hippo_insert_times[$SCALE]}
    faiss=${faiss_insert_times[$SCALE]:-"N/A"}

    if [ "$faiss" != "N/A" ]; then
        winner="FAISS"
        if (( $(echo "$hippo < $faiss" | bc -l) )); then
            winner="Hippocampus ✓"
        fi
    else
        winner="N/A"
    ...
    printf "%-9s | %-11s | %-10s | %s\n" "$SCALE" "$hippo" "$faiss" "$winner"
done

echo
echo "SEARCH PERFORMANCE (ms per query):"
echo "Nodes     | Hippocampus | FAISS      | Winner"
echo "----------|-------------|------------|------------------"
for SCALE in "${SCALES[@]}"; do
    hippo=${hippo_search_times[$SCALE]}
    faiss=${faiss_search_times[$SCALE]:-"N/A"}
...
echo
echo "PURE ALGORITHM PERFORMANCE (excluding embeddings & I/O):"
echo "Nodes     | Hippocampus | FAISS       | Winner"
echo "----------|-------------|-------------|------------------"
for SCALE in "${SCALES[@]}"; do
    hippo_pure=${hippo_pure_search_times[$SCALE]:-"N/A"}
    faiss_pure=${faiss_pure_search_times[$SCALE]:-"N/A"}

    if [ "$faiss_pure" != "N/A" ] && [ "$hippo_pure" != "N/A" ]; then
        winner="FAISS"
        if (( $(echo "$hippo_pure < $faiss_pure" | bc -l) )); then
            winner="Hippocampus ✓"
        fi
    else
        winner="N/A"
    ...
    printf "%-9s | %-11s | %-11s | %s\n" "$SCALE" "$hippo_pure" "$faiss_pure" "$winner"
done
...
echo "=================================================="
echo
echo "Hippocampus uses O(512 × log n) binary search"
echo "FAISS Flat uses O(n) brute force search"
echo
echo "Key Observations:"
echo "  • Hippocampus includes: Bedrock call + file I/O + binary search"
echo "  • FAISS includes: Bedrock call + pure in-memory numpy operations"
echo "  • Pure FAISS search times show true O(n) scaling above"
echo
echo "Expected behavior:"
echo "  • FAISS faster at small scale (100-500 nodes)"
echo "  • Crossover around 500-1000 nodes"
echo "  • Hippocampus competitive despite extra file I/O overhead"
echo
echo -e "${GREEN}✓ Benchmark complete!${NC}"
echo
echo "Cleanup:"
echo "  rm benchmark_hippo_*.bin"
